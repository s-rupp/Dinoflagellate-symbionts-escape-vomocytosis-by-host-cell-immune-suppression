...
```{r}
#load in raw data
dat <- data.frame(read_csv("5a_data.csv"))
head(dat)
```
```{r}
#clean up raw data
...
summary
write_csv(summary, "5a_summary.csv")
```
...
P1<-ggplot(data = summary, aes(x = Algae, y = outcome, group = type)) +
  ...
  geom_errorbar(aes(ymin=down, ymax=up), width=.5, position =position_dodge(.8)) +
...
plot(P1)
ggsave("Figure_5a.pdf",width=16,height=11,units="cm")
```
