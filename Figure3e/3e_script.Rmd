...
```{r}
#load in raw data
dat <- data.frame(read_csv("3e_data.csv"))
head(dat)
```
```{r}
#clean up raw data
...
dat$Halo <- ifelse(dat$Halo>0,1,0)
...
#i <-  unique(dat$Algae)[1]
  ...
  ##Regression model:
  mod <- gam(Halo ~ Treatment + s(Repeat.Date.,bs='re') + s(Well_ID,bs='re'),data=Dat,family=binomial())
...
summary
write_csv(summary, "3e_summary.csv")
```
